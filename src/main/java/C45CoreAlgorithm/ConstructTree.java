...
public class ConstructTree {
  ...
  private TreeNode constructTree(Attribute target, ArrayList<Attribute> attributes,
    ...
    if (rootAttr == null) {
      // Stopping criteria, when there is no more splitting that will give us more entropy, we stop
      // the algorithm.
      String leafLabel = getMajorityLabel(target, instances);
      return new TreeNode(leafLabel);
    }
   ...
   * @return String
   */
  public String getMajorityLabel(Attribute target, ArrayList<Instance> instances)
      throws IOException {
